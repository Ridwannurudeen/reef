...
Reef is **the trust, risk, and capital-allocation layer for autonomous AI agents on Mantle** — and it is live, verified, and reproducible end to end on Mantle Sepolia.
...
- **191 tests passing** (`forge test`) — unit + 3 fuzz/invariant tests + 5 live-mainnet fork tests. The invariants machine-check the two highest-risk accounting surfaces (ReputationBond fund-solvency; AgentVault share-conservation + redemption-solvency under fuzzed deposit/withdraw/donation), mutation-verified to catch a deliberately broken ledger.
- **Live core on Mantle Sepolia (chain 5003)** — `AgentIdentity` `0x4eCE1853623CA801536d319cB9ddE454f5dA6dC7`, `AgentIndex` (rINDEX) `0xC10eCcC78492395f12a8455C8A13471990c53047`, `AdapterRegistry` `0xc1ed73d6707701561df96abbbc37fc7e81f9cc36`; 5 reputation-weighted AgentVaults (526/1052/1578/2631/4210 bps) with a live time-growing-NAV adapter, the index bond-gate active, and Season 0 open — all Mantlescan-verified. UNAUDITED.
- **Registered in Mantle's canonical ERC-8004 registries** — every Reef agent is registered in Mantle's *official* ERC-8004 Identity Registry on Sepolia (`0x8004A818BFB912233c491871b3d84c89A494BD9e`, agents #131–#135), with each agent's Trust Score published to the official Reputation Registry. The reputation Reef computes is portable: any Mantle protocol can read it from the canonical registry without touching a Reef contract.
- **Autonomous, data-grounded agents run unattended** — a VPS cron runs live Z.ai GLM decisions grounded in three real signals (CoinGecko price/momentum, Allora ETH prediction, Nansen smart-money flow), commits each decision's rationale hash on-chain as the receipt evidence (keccak(rationale) == on-chain evidence), and executes real swaps on FusionX V2 (a Mantle-native DEX) in both directions. Feeds at `/api/executions.json`, `/api/reef.json`, `/api/risk.json`.
...
- **Trust Score** — a 0–100 score + letter rating per agent from four on-chain components (reputation 40%, receipt freshness 20%, drawdown 20%, bond 20%); `/api/scores.json`, surfaced on the leaderboard and passport. Recomputed on-chain inside the Allocator (`trustScoreOf`) so allocation is verifiable, not asserted.
- **Agent Passport** — `/agent?id=N`: an institutional per-agent profile (Trust Score + breakdown, on-chain stats, ReefGuard verdict, allocation under the active mandate, signed decision receipts, canonical ERC-8004 link).
- **ReefGuard** — a pure-view on-chain policy gate: `canExecute(agentId, asset, sizeBps) → (allowed, reason)` checks registration, reputation, bond, open disputes, an asset allowlist, and action size. A reference external protocol (`MockProtocol`) gates a real action through it on-chain; an inheritable base (`ReefGuarded`) + a zero-dependency `@reef/sdk` let any Mantle protocol integrate.
...
- **Verify** — ✅ `forge test` green (191); fork tests run against live Mantle mainnet; `SUBMISSION.md` reflects the true live + verified state.
...
Move from paper-mode to a real on-chain product. **Introduces real custody and execution risk — audit required before mainnet TVL.**
