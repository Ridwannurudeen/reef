...
- **Agent Passport** — `/agent?id=N`: an institutional per-agent profile (Trust Score + breakdown, on-chain stats, ReefGuard verdict, allocation under the active mandate, source-labelled decisions, signed receipts, canonical ERC-8004 link).
...
- **Verify** — ✅ `forge test` green (250 passing, 1 skipped); fork tests run against live Mantle mainnet; `SUBMISSION.md` reflects the true live + verified state.
