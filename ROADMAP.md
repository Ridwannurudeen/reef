...
- **More adapters** — ✅ *fBTC + USDe done*: `FbtcAdapter` (Ignition FBTC) and `UsdeAdapter` (Ethena USDe) added (10 unit tests; real Mantle-mainnet addresses on-chain-verified and pinned in `deployments`), broadening the RWA/yield substrate beyond USDY/mETH. Remaining: MI4 (Securitize basket) + mainnet deploy of all adapters (gated on audit).
