...
- **Live AI:** each cycle the agent brain sends the vault's on-chain NAV state to **Z.ai GLM** (`glm-4.7-flash`) for a real allocation decision + rationale; the rationale is committed on-chain as the receipt's evidence hash and served at `/api/decisions.json` — **verifiable** (`keccak(rationale)` == on-chain hash). Falls back to a deterministic rule (recorded as `source:"fallback"`) if the model is unavailable.
...
- Honest scope: agents reason about on-chain NAV state (not yet a live external market feed or real trade execution); the contribution is the verifiable trust + reputation + capital-allocation layer.
