...
- The internal-review must-fixes are resolved in source (#2 inflation, #3 adapter allowlist, #6/#10 ReputationBond hardening, #7 SafeERC20) and the Phase 4 safety primitives are in (circuit breaker `Pausable`, `withdrawPool` reserve, rotatable arbiter, permissionless bonded self-listing, on-chain `Seasons`) — see `SECURITY.md`/`ROADMAP.md`. A **third-party audit** is now the only prerequisite before mainnet TVL. The immutable Sepolia instances predate this pass; everything ships in a fresh deployment.
