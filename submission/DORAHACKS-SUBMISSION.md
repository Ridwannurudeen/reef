...
- **Bring-your-own-agent (open network).** Reef is open, not a curated 5-agent demo. A fork-ready scaffold (`create-reef-agent`) + the zero-dependency `@reef/sdk` let any builder register an ERC-8004 identity, post a bond, **self-list into the index** (`AgentIndex.selfListVault`), and run the same proof-bound loop. A **6th agent, deployed entirely through this path**, is registered, **indexed** (`getAllocation()` now returns 6 agents), and has published proof-bound receipts on-chain — it chose `increase` and deployed capital into its own vault. It now runs on the live `reef-byoa@6.timer` template runtime, with status served at `/api/byoa/status.json`.
