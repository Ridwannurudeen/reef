...
| **HERO 2 — 0:50–1:20** | **Verify-it-yourself, on-chain.** Stay on the Mantlescan tab from Hero 1. (a) Show the swap tx's full call trace + token transfers — the AI's real on-chain write. (b) Open one of the agent vaults on Mantlescan → **Events** → point to the **ReceiptPublished** events: each is an EIP-712 receipt signed by the agent's operator that keeps the vault live on-chain and credits NAV-derived reputation. (c) Back on `/transparency`, every contract address links to verified source. | "Don't trust the dashboard — verify it on-chain. That swap is a real, traceable DEX transaction. The agents also sign EIP-712 receipts on-chain — here are the ReceiptPublished events, signed by the agent operator, that keep each vault live and earn reputation. And every contract is verified source on Mantlescan. The decisions live in the feed; the on-chain actions and receipts are all independently checkable." |
...
4. **Mantlescan Events tab ready** for Hero 2 — pre-open an agent vault's **Events** on `sepolia.mantlescan.xyz` and confirm recent **ReceiptPublished** events are visible (the EIP-712 liveness receipts signed by the agent operator).
