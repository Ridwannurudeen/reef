...
**The thesis in one line:** "Which autonomous AI agents can you trust with capital?" Reef makes that answer an on-chain primitive — portable ERC-8004 identity, source-labelled decisions, EIP-712 receipts, NAV-derived reputation, and a public 0–100 Trust Score any Mantle protocol can read.
...
|---|---|---|
| **0:00–0:12** | Landing page `reef.gudman.xyz`. Pan the hero: "Verifiable AI yield agents," eyebrow "Built on Mantle · The Financial Turing Test," "Live on Mantle Sepolia" pill. | "AI agents are starting to manage real money on-chain — and almost none of it is verifiable. Reef fixes that on Mantle with source-labelled decisions, signed receipts, and proof records you can check yourself. We turn the hard question — can you actually trust an AI with capital? — into the Financial Turing Test. Built for the Mantle Turing Test, AI×RWA track." |
...
| **HERO 1 — 0:25–0:50** | **The AI writing on-chain — atomically.** Open `reef.gudman.xyz/api/proofbound.json` (raw JSON). Highlight the newest pass: per agent `source: "glm"`, `model: "glm-4.7-flash"`, the `rationale`, the `action`/`moveStatus`, and the receipt `txHash` with `bound: true`. Copy that `txHash` → open on Sepolia Mantlescan → show the confirmed `publishReceipt`. (Optional: point to the 6th BYOA agent's record, which chose `increase` and **deployed** capital.) | "Here's the live loop. A cron runs a real GLM model — glm-4.7-flash — over live signals and each vault's on-chain NAV, and in one atomic pass it gates the decision through ReefGuard, moves vault capital through a vetted strategy adapter, and binds the rationale into an on-chain receipt. Here's that receipt transaction, confirmed on Mantlescan. That's an autonomous AI agent making a real, gated, proof-bound on-chain write — and one of these agents was brought by an outside builder through our SDK." |
| **HERO 2 — 0:50–1:20** | **Verify-it-yourself: a matched proof record.** Open `reef.gudman.xyz/api/proofs.json`. Pick one agent with `proofStatus: "matched"`, then highlight its `reasoning` and `evidenceHash`. (a) Drop that exact `reasoning` string into a keccak256 tool and show the hash. (b) Open that agent's vault on Mantlescan → **Read Contract** → call **`lastReceiptEvidenceHash`** (bytes32). (c) Hold them side by side: `keccak256(reasoning)` == `lastReceiptEvidenceHash`. If the latest record is `liveness-only`, wait for the next receipt tick or narrate it as a liveness receipt, not a rationale proof. | "Don't trust the dashboard — verify it yourself. Here's a matched proof record. I take these exact words, hash them with keccak256, and that hash is identical to the `lastReceiptEvidenceHash` this agent committed on-chain. The source field tells you whether the bound rationale came from GLM or the deterministic fallback. Cadence-only receipts are labelled separately; those prove liveness, not rationale binding." |
...
- **Only matched proof records demonstrate rationale binding**: `proofStatus: "matched"` means `keccak256(reasoning)` == the vault's `lastReceiptEvidenceHash`. `proofStatus: "liveness-only"` means the receipt is a cadence/liveness record, not a rationale proof. Don't claim every latest receipt is rationale-bound.
...
- Live proof-bound feed `/api/proofbound.json` (+ `/api/proofs.json`): per-agent `source, model, action, moveStatus, rationale, seq, evidenceHash, txHash, bound, proofStatus` — written by the sole-publisher `agents/scripts/proofbound_rebalance.py` every ~10 min. (`/api/executions.json` is the legacy FusionX-swap feed and is no longer the live path — do not point judges at it.)
- 6th agent is BYOA: deployed via `create-reef-agent/`, registered + index-listed (`getAllocation()` returns 6 agents), chose `increase` and deployed capital (receipt seq=3).
