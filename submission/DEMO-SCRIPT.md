...
| **HERO 1 — 0:25–0:50** | **The AI writing on-chain — atomically.** Open `reef.gudman.xyz/api/proofbound.json` (raw JSON). Highlight the newest pass: per agent `source: "glm"`, `model: "glm-4.7-flash"`, the `rationale`, the `action`/`moveStatus`, and the receipt `receiptTx` with `proofStatus: "bound"`. Copy that `receiptTx` → open on Sepolia Mantlescan → show the confirmed `publishReceipt`. (Optional: point to the 6th BYOA agent's own `/api/byoa/6/proofbound-agent.json` runtime feed, which chose `increase` and **deployed** capital.) | "Here's the live loop. A cron runs a real GLM model — glm-4.7-flash — over live signals and each vault's on-chain NAV, and in one atomic pass it gates the decision through ReefGuard, moves vault capital through a vetted strategy adapter, and binds the rationale into an on-chain receipt. Here's that receipt transaction, confirmed on Mantlescan. That's an autonomous AI agent making a real, gated, proof-bound on-chain write — and one of these agents was brought by an outside builder through our SDK." |
...
3. **Confirm the AI feed is fresh:** refresh `/api/proofbound.json` and verify the newest record is `source: "glm"` with a non-null `receiptTx` and `proofStatus: "bound"`. If only `fallback` records show, wait for the next cron cycle or narrate the fallback honestly.
