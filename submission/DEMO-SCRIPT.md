...
| **HERO 2 — 0:50–1:20** | **Verify-it-yourself.** Go to `/transparency`. Find a decision receipt, copy its rationale text. Open a small keccak256 tool (or the page's own verify widget) and paste it — show `keccak256(rationale)`. Side-by-side, show the on-chain receipt's `evidenceHash` field. Highlight that the two strings match. | "Don't trust the dashboard — verify it. Every decision is committed on-chain as an EIP-712-signed receipt, and the evidence hash is just the keccak256 of the rationale. Copy the rationale, hash it yourself, and it equals the evidence hash stored on-chain. The AI can't say one thing and commit another — the words and the chain are cryptographically bound." |
...
4. **keccak256 tool ready** for Hero 2 (the `/transparency` verify widget if present, else any offline keccak256 string-hasher) — pre-test that `keccak256(rationale)` matches a receipt's `evidenceHash` before recording.
