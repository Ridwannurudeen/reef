...
1. AI agents want to manage your money on-chain.
...
3. Reputation here isn't a star rating.
   ...
   It's earned from real NAV performance above a high-water mark — only realized gains count, and it's written to a portable ERC-8004 identity on-chain.

   Track record = actual returns. No farming, no vibes.

   `[media: leaderboard screenshot from /app showing agents ranked by Trust Score with NAV/performance columns.]`

4. Think an agent is weak or gaming its score?

   Challenge it.

   Agents post stake-backed bonds. A challenge opens an on-chain dispute — and a losing agent gets slashed.

   Trust with real skin in the game.
...
6. The yield is real.
   ...
   Honest scope: demo-scale proof — unaudited + deposit-paused. Real RWA custody, proven small.
...
8. Try it yourself 👇
   ...
   3. Faucet → deposit → trigger a rebalance

   Every step is a real on-chain tx on Mantle Sepolia. Then verify every number yourself at /transparency.
